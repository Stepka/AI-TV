...
export default function BrandPhrasesLibrary({ token, userData, channel, onSave }) {
  ...
  const generateAudio = async (ad) => {
    ...
    if (!res.ok) throw new Error(t("phrases.generateBrandSpeechFailed"));
  ...
  const addAd = async () => {
    ...
    await fetch(`${API_URL}/media/add_prerecord_brand_phrase`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const updateAd = async (ad) => {
    ...
    await fetch(`${API_URL}/media/update_prerecord_brand_phrase`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleDeleteAd = async (ad) => {
    if (!window.confirm(t("phrases.confirmDeleteBrand"))) return;

    try {
      const res = await fetch(`${API_URL}/media/delete_prerecord_brand_phrase`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
  ...
  return (
    <div style={{ 
      ...
      <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 20 }}>
        <h2>{t("phrases.brandTitle")}</h2>
        <span>{t("audioLibrary.availableGenerations", { count: userData?.prerecord_welcome_num })}</span>
        
        <AppButton onClick={addAd} style={{ marginTop: 8, width: "fit-content" }}>{t("phrases.addBrand")}</AppButton>
