...
def generate_ultra_short_text(user_uid: str, channel_uid: str) -> str:
    ...
    phrases_examples = [
        f"Мы прололжаем в {meta["name"]}",
        f"Я рад/рада, что вы с нами в {meta["name"]}", 
        f"Я рад/рада, что вы выбрали {meta["name"]}",
        f"Мы рады вас видеть в {meta["name"]}", 
        f"Вы в {meta["name"]}", 
        f"Вы по-прежнему в {meta["name"]}" 
    ]
...
Сделай очень короткий текст перехода к слудующей песне, вроде: {random.choice(phrases_examples)}
...
— от {'мужского' if meta["voice"]["sex"] == "male" else 'женского'} пола 
— разговорный стиль
— весь текст должен быть в 1 предложение, до 10 слов - это очень важно
"""
